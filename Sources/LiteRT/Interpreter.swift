...
public final class Interpreter {
    ...
    public var signatureKeys: [String] {
        guard let signatureKeys = _signatureKeys else {
            ...
            let keys: [String] = (0..<signatureCount).map {
                guard let signatureNameCString = TfLiteInterpreterGetSignatureKey(self.cInterpreter, Int32($0))
                else { return "" }
    ...
    public convenience init(modelPath: String, options: Options? = nil, delegates: [Delegate]? = nil) throws {
        guard let model = Model(filePath: modelPath)
        else { throw InterpreterError.failedToLoadModel }
    ...
    public convenience init(modelData: Data, options: Options? = nil, delegates: [Delegate]? = nil) throws {
        guard let model = Model(modelData: modelData)
        else { throw InterpreterError.failedToLoadModel }
    ...
    private init(model: Model, options: Options? = nil, delegates: [Delegate]? = nil) throws {
        guard let cInterpreterOptions = TfLiteInterpreterOptionsCreate()
        else { throw InterpreterError.failedToCreateInterpreter }
        ...
        guard let cInterpreter = TfLiteInterpreterCreate(model.cModel, cInterpreterOptions)
        else { throw InterpreterError.failedToCreateInterpreter }
    ...
    public func invoke() throws {
        guard TfLiteInterpreterInvoke(cInterpreter) == kTfLiteOk
        else { throw InterpreterError.allocateTensorsRequired }
    }
    ...
    public func copy(_ data: Data, toInputAt index: Int) throws -> Tensor {
        ...
        guard status == kTfLiteOk
        else { throw InterpreterError.failedToCopyDataToInputTensor }
    ...
    public func allocateTensors() throws {
        guard TfLiteInterpreterAllocateTensors(cInterpreter) == kTfLiteOk
        else { throw InterpreterError.failedToAllocateTensors }
    }
    ...
    public func signatureRunner(with key: String) throws -> SignatureRunner {
        guard signatureKeys.contains(key)
        else {
