...
/// as a `SignatureRunner` instance is still in use, its associated `Interpreter` instance will not be deallocated.
public final class SignatureRunner {
    ...
    public var inputs: [String] {
        guard let inputs = _inputs else {
            ...
            let ins: [String] = (0..<inputCount).map {
                guard let inputNameCString = TfLiteSignatureRunnerGetInputName(self.cSignatureRunner, Int32($0))
                else { return "" }
    ...
    public var outputs: [String] {
        guard let outputs = _outputs else {
            ...
            let outs: [String] = (0..<outputCount).map {
                guard let outputNameCString = TfLiteSignatureRunnerGetOutputName(self.cSignatureRunner, Int32($0))
                else { return "" }
    ...
    ///
    /// - Warning: Never let the interpreter hold a strong reference to the signature runner to avoid retain cycles.
    ...
    internal init(interpreter: Interpreter, signatureKey: String) throws {
        guard let signatureKeyCString = signatureKey.cString(using: String.Encoding.utf8),
              let cSignatureRunner = TfLiteInterpreterGetSignatureRunner(interpreter.cInterpreter, signatureKeyCString)
        else {
    ...
    public func invoke(with inputs: [String: Data]) throws {
        ...
        guard TfLiteSignatureRunnerInvoke(self.cSignatureRunner) == kTfLiteOk
        else {
    ...
    public func allocateTensors() throws {
        guard isTensorsAllocationNeeded else { return }
        guard TfLiteSignatureRunnerAllocateTensors(self.cSignatureRunner) == kTfLiteOk
        else {
    ...
    private func tensor(named nameInSignature: String, withType type: TensorType) throws -> Tensor {
        guard let nameInSignatureCString = nameInSignature.cString(using: String.Encoding.utf8)
            ...
            throw SignatureRunnerError.failedToGetTensor(tensorType: type.rawValue, nameInSignature: nameInSignature)
        }
        var cTensorPointer: CTensor
        switch type {
        case .input:
            cTensorPointer = UnsafePointer(TfLiteSignatureRunnerGetInputTensor(self.cSignatureRunner, nameInSignatureCString))
        case .output:
            cTensorPointer = TfLiteSignatureRunnerGetOutputTensor(self.cSignatureRunner, nameInSignatureCString)
        }
        guard let cTensor = cTensorPointer
        else {
            throw SignatureRunnerError.failedToGetTensor(tensorType: type.rawValue, nameInSignature: nameInSignature)
        }
        guard let bytes = TfLiteTensorData(cTensor)
        else {
            throw SignatureRunnerError.allocateTensorsRequired
        }
        guard let dataType = Tensor.DataType(type: TfLiteTensorType(cTensor))
        else {
